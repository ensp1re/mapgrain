...
Published npm CLI: `mapgrain@0.2.5`. Requires Node 24 or newer. That package validates architecture, workflow, sequence, data-flow, and lifecycle documents from this skill. Do not pin an unversioned latest tag and do not overwrite `0.1.0` or `0.2.0`.
...
- Run layout with `npx mapgrain@0.2.5 layout diagram.json` so ELK writes positions. Do not pick x/y yourself.
...
2. `npx mapgrain@0.2.5 validate diagram.json`
3. `npx mapgrain@0.2.5 layout diagram.json` (add `--rearrange` only when the user asked to re-layout).
4. `npx mapgrain@0.2.5 view diagram.json -o diagram.html`
...
npx mapgrain@0.2.5 export diagram.json --format png -o diagram.png
```
