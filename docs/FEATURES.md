...
| Sequence diagrams | shipped | Lifelines, ordered messages, and alt/opt fragments. npm `mapgrain@0.2.4` validates them; `0.1.0` still rejects them |
...
| Agent skill | partial | Install paths proven and a worked example of every diagram kind ships with it; live agent prompt runs untested; matching CLI is `npx mapgrain@0.2.4` |
