...
| Workflow / sequence / data-flow / lifecycle modes | implemented | yes | npm `mapgrain@0.2.4` validates all five modes including sequence alt/opt fragments; historical `0.1.0` does not |
| Blank / open / templates / agent setup on start | implemented | yes | Start surface leads with a searchable template gallery covering all five kinds; agent setup is copy, not a picker with troubleshooting |
| Connection editing: select, caption, direction, endpoints, line shape, delete | implemented | yes | F071: React Flow's dropped edge changes were why none of it was reachable; endpoints and line shape are new operations |
...
| CLI validate/layout/view/export/doctor/studio | implemented | yes | npm `mapgrain@0.2.4` is last published and validates all five modes outside the monorepo |
| Agent skill install + live tasks across agents | partial | partial | Install paths tested; live agent tasks untested; skill uses `npx mapgrain@0.2.4` and must not send sequence fixtures to `npx mapgrain@0.1.0` |
