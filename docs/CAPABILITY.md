...
| Workflow / sequence / data-flow / lifecycle modes | implemented | yes | npm `mapgrain@0.2.3` validates all five modes including sequence alt/opt fragments; historical `0.1.0` does not |
...
| CLI validate/layout/view/export/doctor/studio | implemented | yes | npm `mapgrain@0.2.3` is last published and validates all five modes outside the monorepo |
| Agent skill install + live tasks across agents | partial | partial | Install paths tested; live agent tasks untested; skill uses `npx mapgrain@0.2.3` and must not send sequence fixtures to `npx mapgrain@0.1.0` |
