...
| --- | --- | --- | --- |
| Lint, typecheck, test, or build failure | Non-zero `pnpm verify` | Keep task `active`, fix, rerun `verify ID` | Attempt JSON under `docs/runs/` |
...
Logs under `docs/runs/` are gitignored. Durable summaries live on the task record.
