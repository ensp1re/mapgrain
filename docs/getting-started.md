...
```sh
npx mapgrain@0.2.2 studio diagram.json
```
...
npx mapgrain@0.2.2 view diagram.json --lang uk -o view.html
npx mapgrain@0.2.2 export diagram.json --format svg -o diagram.svg
npx mapgrain@0.2.2 export diagram.json --format card --view overview -o card.png
npx mapgrain@0.2.2 watch diagram.json --once --format html -o view.html
npx mapgrain@0.2.2 doctor
```

From this checkout, `pnpm mapgrain` is the development command (source CLI `0.2.2`). Last published package: `npx mapgrain@0.2.2`. Historical architecture/workflow package: `npx mapgrain@0.1.0`, which does not validate sequence, data-flow, or lifecycle fixtures. `npx mapgrain@0.2.0` remains on npm.
...
| Symptom | What to try |
| --- | --- |
| `unsupported schemaVersion` | The file is newer than this CLI. Upgrade `npx mapgrain@0.2.2` or export from a matching editor. |
...
| Skill installed, CLI missing | Install Node 24+ and retry `npx mapgrain@0.2.2 doctor`. |
| `npx mapgrain@0.1.0` rejects sequence, data-flow, or lifecycle | Use `npx mapgrain@0.2.2` or `pnpm mapgrain` from this checkout. |
