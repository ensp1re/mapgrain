...
import { defaultFont, type Point, type Scene } from "@mapgrain/scene";
...
export function renderSvg(scene: Scene, theme: Theme): { svg: string; width: number; height: number } {
    ...
    .map((edge) => {
      ...
      const head =
        from && to
          ...
          : "";
      return `<g data-kind="edge" data-id="${escapeXml(edge.id)}" fill="${tokens.edge}" stroke="${tokens.edge}">
  ...
  const nodes = scene.nodes
    .map((node) => {
      ...
      const textY = node.rect.y + oy + (node.rect.height - node.label.height) / 2;
      const lines = node.label.lines
        .map(
          (line, index) =>
            `<text x="${n(textX)}" y="${n(textY + index * line.height)}" text-anchor="middle" dominant-baseline="hanging" fill="${tokens.text}" font-family="${escapeXml(font.family)}" font-size="${font.size}">${escapeXml(line.text)}</text>`,
        )
        .join("\n  ");
