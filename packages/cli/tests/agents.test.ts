...
test("npx mapgrain@0.1.0 validates, layouts, and views in an empty directory", { timeout: 90_000 }, async () => {
  ...
  const layout = await run("npx", ["--yes", PUBLISHED_CLI, "layout", input, "-o", laid], dest);
  ...
  const view = await run("npx", ["--yes", PUBLISHED_CLI, "view", laid, "-o", html], dest);
